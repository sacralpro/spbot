...
def process_name_for_standard_bot(message, lang):
    ...
    idea_prompt = "Опишите логику вашего бота. Например: В боте должно быть 3 кнопки меню: О компании, наши услуги, контакты. Услуги: заказать веб-сайт, заказать моб-приложение и так далее.." if lang == 'Русский' else "Please, describe the logic of your bot. For example: “The bot should have 3 menu buttons: About Us, Our Services, Contact. Services: order a website, order a mobile app, etc.."
...
def process_logic_for_standard_bot(message, lang):
    # Here we can save or process the logic, if necessary
    # You may store this data in order_details if needed
...
def process_name_for_consultation(message, lang):
    ...
    bot.register_next_step_handler(message, process_question_for_consultation, lang)

def process_question_for_consultation(message, lang):
    # Confirmation message
    if lang == 'Русский':
        bot.send_message(message.chat.id, "Наш специалист свяжется с Вами.")  # Confirmation in Russian
    else:
        bot.send_message(message.chat.id, "Our specialist will contact you.")  # Confirmation in English

    # Return to start
    start(message)  # Show the start command to return to language selection
...
def process_budget(message, lang, is_mobile_app):
    ...
    # Возврат к началу
    start(message)  # Показать команду начала для возврата к выбору языка
